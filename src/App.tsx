...
import { listRuns, saveRun, getRun, deleteRun, type HistoryEntry } from "@/lib/history";
...
export default function App() {
  ...
  // Load persisted run history once on mount.
  useEffect(() => {
    setHistory(listRuns());
  }, []);

  // Persist a completed run and refresh the history list.
  const persistRun = (finalState: GlobalState) => {
    setHistory(saveRun(finalState));
  };

  // Reload a past run into the workbench without re-running the pipeline.
  const loadRun = (id: string) => {
    ...
    setQuestion(saved.research_question || question);
    setStep("completed");
    setProgressPercent(100);
    setSelectedHypIndex(0);
    setSelectedExpIndex(0);
    setLogs([
      {
        timestamp: new Date().toLocaleTimeString(),
        agent: "Orchestrator",
        message: `Loaded saved run: "${saved.research_question}"`,
        type: "success",
      },
    ]);
  };
        ...
        <div className="flex flex-col min-h-screen">
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-stretch max-w-8xl w-full mx-auto pb-28">
            ...
            <aside className="lg:col-span-3 flex flex-col gap-5 h-full">
              ...
              {/* Run history */}
              {history.length > 0 && (
                <div className="p-5 bg-zinc-950 border border-zinc-900 rounded-xl flex flex-col gap-3">
                  <h3 className="text-xs font-bold text-white uppercase tracking-wider font-mono flex items-center gap-1.5">
                    <History className="w-3.5 h-3.5 text-purple-400" />
                    Run History
                    <span className="text-[9px] text-zinc-600 font-normal">({history.length})</span>
                  </h3>

                  <div className="flex flex-col gap-1.5 max-h-52 overflow-y-auto scrollbar-thin -mr-2 pr-2">
                    {history.map((h) => (
                      <div
                        key={h.id}
                        className={`group flex items-start gap-2 px-2.5 py-2 rounded-lg border transition-all cursor-pointer ${
                          globalState?.session_id === h.id
                      ...
                      >
                        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                          <span className="text-[11px] text-white font-medium truncate leading-snug">{h.title}</span>
                          <span className="text-[9px] text-zinc-500 font-light truncate">{h.question}</span>
                          <span className="text-[8px] text-zinc-600 font-mono flex items-center gap-1 mt-0.5">
